...
export async function synthesizeText(text: string): Promise<string | undefined> {
  try {
    ...
    console.warn("TTS fallido (esto es normal en algunas regiones):", error);
    return undefined;
  }
}
...
export async function getChatResponse(message: string) {
  ...
  try {
    ...
    try {
      // Intentamos generar audio pero no bloqueamos si falla (común en free tier o límites de cuota)
      audioData = await synthesizeText(text);
    } catch (e) {
      console.warn("Audio omitido por cuota.");
    }
    ...
    if (error?.message?.includes('429')) {
      return { 
        text: "¡Ay, viditay! Me cansé un poquito de tanto hablar. Esperemos un minutito, ¿ya, tesoro?",
        sources: [] 
      };
    }
    throw error;
